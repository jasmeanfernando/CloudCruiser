...
<% } else { %>
	<% try {
		...
		//Create query.
		String aggregateSql = "SELECT SUM(F.TotalPrice) AS TotalPriceForCustomer, COUNT(R.ReservationNumber) AS NumberOfReservations " +
                              "FROM Reservation R " +
                              "JOIN Flight F ON R.FlightNumber = F.FlightNumber " +
                              "WHERE R.CustomerID = ?";
        
		PreparedStatement aggregateStatement = con.prepareStatement(aggregateSql);
        aggregateStatement.setString(1, email);
        ...
        detailStatement.setString(1, email);
        ResultSet detailResultSet = detailStatement.executeQuery();

        if (aggregateResultSet.next()) {
            price = aggregateResultSet.getDouble("TotalPriceForCustomer");
            res = aggregateResultSet.getInt("NumberOfReservations");
        }
        
        out.print("<h2>Revenue Report By Customer</h2>");
